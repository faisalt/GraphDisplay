...
* - sort out left panel stuff - y axis
* - sort out what happens if other panels are added
* - reset individual panels while preserving the row orders, etc. of the other (might need to assign an id to each row and col for this to work)
...
var REQUEST_ROW_LENGTH		= "REQUEST_COLUMN_LENGTH";
var REQUEST_ALLROWS			= "REQUEST_ALLCOLUMNS";
...
io.sockets.on('connection', function (socket) {
	...
	socket.on(REQUEST_ROW_LENGTH, function(message, callback) {
		var clength = DataSetObject.TotalMaxRows();
		callback(clength);
	});
	socket.on(REQUEST_ALLROWS, function(message, callback) {
		var cdata = DataSetObject.AllRowValues();
		callback(JSON.stringify(cdata));
	});
