...
// Set the channels so that we can handle the Unity3D client for Augmented Reality interactions.
var UNITY_CHANNEL						= "UNITY_CHANNEL";
var GRAPH_CHANNEL						= "GRAPH_CHANNEL";
...
io.sockets.on('connection', function (socket) {
	...
	socket.on("UPDATE_GUI", function(client) {
		...
		socket.broadcast.emit(DATASET_X_LABEL_UPDATE, JSON.stringify({columns:DataSetObject.getColumnLabelWindow(), lockedColumns : LockedData.getActualColumnIndices(), lockedRows : LockedData.getActualRowIndices()}));
		...
		socket.emit(DATASET_Y_LABEL_UPDATE, JSON.stringify({rows:DataSetObject.getRowLabelWindow(), lockedRows : LockedData.getActualRowIndices(), lockedColumns : LockedData.getActualColumnIndices()}));
		socket.broadcast.emit(DATASET_Y_LABEL_UPDATE, JSON.stringify({rows:DataSetObject.getRowLabelWindow(), lockedRows : LockedData.getActualRowIndices(), lockedColumns : LockedData.getActualColumnIndices()}));
	...
	socket.on(REQUEST_ALLCOLUMNS, function(message, callback) {
		...
		callback(JSON.stringify({data:cdata, xindex:send_xindex, lockedColumns : LockedData.getActualColumnIndices(), col_length:DataSetObject.TotalMaxColumns(), lockedRows : LockedData.getActualRowIndices()}));
	});
	...
	socket.on(REQUEST_ALLROWS, function(message, callback) {
		...
		callback(JSON.stringify({data:rdata, yindex:send_yindex, lockedRows : LockedData.getActualRowIndices(), row_length:DataSetObject.TotalMaxRows(), lockedColumns : LockedData.getActualColumnIndices()}));
	});
	...
	socket.on(UPDATE_DATASET_SCROLLX, function(message, callback) {
		...
		socket.to(GRAPH_CHANNEL).emit(DATASET_WINDOW_UPDATE, sendBigJSONdata({dataset:true, rowcolors:true, minz:true, maxz:true, animationTime:true}));
		socket.to(UNITY_CHANNEL).emit(UNITYCLIENT_DATASET_WINDOW_UPDATE, {data:prepareDataWindow_Unity(DataSetObject.getDataWindow()), minz:DataSetObject.DataMinValue(), maxz:DataSetObject.DataMaxValue()});
		socket.to(GRAPH_CHANNEL).emit(DATASET_X_SCROLLBAR_UPDATE, JSON.stringify({xindex:DATA_INDEX.getXScrollIndex(), xlabels:DataSetObject.getColumnLabelWindow()}));
	});
...
function LockedData() {
	...
	this.addLockedColumns=function(index) {
		...
		for (var row = 0; row < DataSetObject.TotalMaxRows(); ++row) {
			data[row][globalColIndex].locked = true;
...
function DataSetObject(csvfile, xmlfile) {	
	...
	var dataValObject = function(id, val, x, y) {
		...
		this.locked = false; // Whether this bar has been locked
		this.init = function() { }
	}
...
function filterSingleDataPoint() {
	...
	for(var i=0; i<filterCoordinates.length; i++) {
		...
		var actual_x = filterCoordinates[i][0];
		var actual_y = filterCoordinates[i][1];
		if(mywindow[actual_x][actual_y].locked == false) {
	...
	if(PRESS_COMPARE_COUNTER == 2 && (COMPARE_TIME_2 - COMPARE_TIME_1) < FILTER_COMPARISON_INTERVAL) {
		...
		if(filterCoordinates.length > 1) {
			...
			(filterCoordinates[0][1] == 9 && filterCoordinates[1][1] == 9 && filterCoordinates[1][0] == 8) || (filterCoordinates[1][1] == 9 && filterCoordinates[0][1] == 9 && filterCoordinates[0][0] == 8)){
				...
				filterCompare("COMPARE_ROW", filterCoordinates[0][0], filterCoordinates[1][0], filterCoordinates[0][1], filterCoordinates[1][1]);
			}
...
/** Unity can't handle giant JSON data strings over websocket, so we need to strip down some of the values and send the basics. */
function prepareDataWindow_Unity(datawindow) {
	var u_array=Array();
	for(var i=0; i<datawindow.length; i++) {
		for(var j=0; j<datawindow[i].length; j++) {
			u_array.push(datawindow[i][j].val);
		}
	}
	return u_array;
}
